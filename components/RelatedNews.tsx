...
export default function RelatedNews({ currentSlug, category, allNews }: RelatedNewsProps) {
  ...
  // Fungsi pembantu aman untuk mengekstrak string dari field teks/objek manapun (Anti-Crash)
  ...
  const related = allNews.filter((post: any) => {
    ...
    // Filter Keamanan: Menepis konten video dan link youtube agar tidak bocor ke seksi ini
  ...
  const fallback = allNews.filter((post: any) => {
    ...
    return (
      ...
      !isVideoFormat &&
      !hasYoutube &&
      !related.some((r: any) => extractString(r?.slug) === postSlug)
    );
  });
